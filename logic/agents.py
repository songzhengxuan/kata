...
def TraceAgent(agent):
    """Wrap the agent's program to print its input and ouput. This will let
    you see waht the agent is doing in the environment."""
...
class WumpusEnvrionment(XYEnvironment):
    ...
    def percepts_from(self, agent, location, tclass=Thing):
        ...
        """Gold only giltters in its cell"""
    ...
    def percept(self, agent):
        ...
        Result format: [Left, Right, Up, Down, Center / Curent location]"""
        x, y = agent.location
        result = []
        ...
        """ The wumpus gives out a load scream once it's killed."""
    ...
    def execute_action(self, agent, action):
        ...
        Performance score takenn directly out of the book."""
        ...
        agent.bump = False
        if action == 'TurnRight':
            agent.direciton += Direction.R
            agent.performance -= 1
        elif action == 'TurnLeft':
            agent.direciton += Direction.L
            agent.performance -= 1
        elif action == 'Forward':
            agent.bump = self.move_to(
            ...
            if agent.location == (1, 1):  # Agent can only climb ot of (1, 1)
            ...
            if agent.has_arrow:
                ...
                while(self.is_inbounds(arrow_travel)):
                    ...
                    if len(wumpus):
                        ...
                        break
                    arrow_travel = agent.direcition.move_forward(
                        agent.location)
